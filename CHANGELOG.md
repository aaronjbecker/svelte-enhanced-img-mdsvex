...
- feat: Fork of `@sveltejs/enhanced-img` v0.9.2 with Markdown/MDSvex support
- feat: Add support for `.md` file processing alongside `.svelte` files
- feat: Add `convertMarkdownSyntax` option to auto-convert `![alt](src)` syntax
- feat: Add `convertRegularImgTags` option to auto-convert `<img>` tags in markdown
- feat: Regex-based parser for markdown to avoid Svelte parse errors
- feat: Graceful handling of remote URLs and public folder assets
- feat: Enhanced script injection for markdown files

---

## Upstream Changes from @sveltejs/enhanced-img
