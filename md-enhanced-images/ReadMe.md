...
This is a Vite plugin for SvelteKit projects that provides enhanced image optimization for both `.svelte` and `.md` (mdsvex) files. It is adapted from the official `@sveltejs/enhanced-img` plugin but extended to support the specific needs of a Markdown-based blog.
...
```typescript
import { mdEnhancedImages } from './src/lib/blog/md-enhanced-images';
